...
#print("segmented_masks_list is ", segmented_masks_list)

# list fluorescence subfolders in the fluo_channels folder
#fluo_channels_list = os.listdir(input_dir + 'fluo_channels/')
...
for fluo_tmp in fluo_channels_list:
    ...
    ###############################################
    ### extract median fluorescence of background
    ###############################################

    # Create empty result table
    background_results = pd.DataFrame(columns=['Timepoint', 'Mask_nb', fluo_tmp_name + '_median'])
    ...
    for tp in range(len(segmented_masks_list)):
        ...
        # Keep fluorescence only in the background, replace all other fluorescence by 0
        img_tmp_fluo = np.where(img0 != 0, 0, img0_fluo)

        # Measure median fluorescence in the background
        fluo_median = np.quantile(img_tmp_fluo[np.nonzero(img_tmp_fluo)], 0.5)

        # save results in a panda dataframe
        background_results.loc[tp] = [tp, # timepoint
                                    0, # mask number
                                    fluo_median]

    avg_fluo = background_results[fluo_tmp_name + '_median'].median()
    delta_fluos = background_results[fluo_tmp_name + '_median'] - avg_fluo

    ###############################################
    ### use median fluorescence of background to
    ### normalise images before extracting fluo ft
    ###############################################
    ...
    for tp in range(len(segmented_masks_list)):
        ...
        for mask_tmp in unique0[1:]:
            ...
            # Normalise pixels based on background
            img_tmp_normalised = img_tmp_fluo - delta_fluos[tp]

            # Measure total, average, 1st quartile, 3rd quartile, min and max fluorescence in the current cell mask
            fluo_mean = img_tmp_normalised[np.nonzero(img_tmp_fluo)].mean()
            fluo_median = np.quantile(img_tmp_normalised[np.nonzero(img_tmp_fluo)], 0.5)
            fluo_1quart = np.quantile(img_tmp_normalised[np.nonzero(img_tmp_fluo)], 0.25)
            fluo_3quart = np.quantile(img_tmp_normalised[np.nonzero(img_tmp_fluo)], 0.75)
            fluo_sum = img_tmp_normalised[np.nonzero(img_tmp_fluo)].sum()
            fluo_min = img_tmp_normalised[np.nonzero(img_tmp_fluo)].min()
            fluo_max = img_tmp_normalised[np.nonzero(img_tmp_fluo)].max()
